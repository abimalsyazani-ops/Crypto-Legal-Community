...
const officialSocialLinks = {
  email: "mailto:cryptolegalcommunity@gmail.com",
...
const footerSocials = [
  { label: "Email", href: officialSocialLinks.email },
  ...
  return (
    <header className="site-header">
      <a className="brand" onClick={() => navigate("/")}>
        <img src="/clc-brand-logo.png" alt="Logo Crypto Legal Community" />
...
function HelpCenterPage() {
  return <section className="help-page"><h1>Hubungi <span>CLC</span></h1><p className="help-address">Crypto Legal Community<br />Media Hukum, Crypto, dan Ekonomi Digital<br />Indonesia</p><p className="help-intro">Untuk menghubungi kami via email, isi kolom pada formulir, atau kirimkan email ke <a href={officialSocialLinks.email}>cryptolegalcommunity@gmail.com</a></p><form className="help-form" action="mailto:cryptolegalcommunity@gmail.com" method="post" encType="text/plain"><label>Name<input name="name" placeholder="Name" required /></label><label>Email<input name="email" placeholder="Email" type="email" required /></label><label>No Hp<input name="phone" placeholder="No Hp" /></label><label>Pesan<textarea name="message" placeholder="Pesan" required /></label><button className="primary">Kirim Pesan</button></form><p className="help-note">CLC tidak pernah meminta seed phrase, private key, kode OTP, atau akses wallet pengguna.</p></section>;
}
...
function AdminPage({ admin, setAdmin, drafts, saveDraft, dataStatus, remoteArticles, remoteEvents, session, setSession, profile, setProfile, installPrompt, setInstallPrompt }: any) {
  ...
  return (
    <main className="admin">
      <aside>
        <img src="/clc-brand-logo.png" alt="CLC" />
